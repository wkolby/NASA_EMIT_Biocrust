...
#####################################TEST#######################################
file_list <- list.files(paste(github_dir,'/Data/Wavelength_Calibration_2025/UA_ASD_FieldSpec3/4.22.25/',sep=''), full.names = T, pattern = ".asd")
dataset <- c()
for(i in 1:length(file_list)){
  ...
  ds=cbind(wvl,data,type)
  dataset <- rbind(dataset,ds)
}

#Name the columns
colnames(dataset) <- c("wavelength","reflectance","rep")
...
ggsave(paste(github_dir,'/figures/Wavelength_Calibration_UA_ASD_FieldSpec3.png',sep=''),dpi=300,width=180,height=120,units='mm')
